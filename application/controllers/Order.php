		...
		switch ($action) {
			case 'cancel_order':
				...
				if (($data->payment_method == 1 && $data->order_status == 1) || ($data->payment_method == 2 && $data->order_status == 1)) {
					...
					$response = array('code' => 200, 'success' => TRUE, 'message' => 'Order Cancelled');
				} else {
					$response = array('code' => 200, 'error' => TRUE, 'message' => 'Order cannot be cancelled');
				}
				break;
			case 'delete_order':
				$id = $this->input->post('id');
				...
				if (($data->payment_method == 1 && ($data->order_status == 5 || $data->order_status == 4)) || ($data->payment_method == 2 && ($data->order_status == 4 || $data->order_status == 3))) {
					...
					$response = array('code' => 200, 'success' => TRUE, 'message' => 'Order deleted');
				} else {
					$response = array('code' => 200, 'error' => TRUE, 'message' => 'Order cannot be deleted');
				}
				break;
		}
