...
class Register extends CI_Controller
{
	public function __construct()
	{
		...
		$this->load->model('Model_register', 'register'); //load model register dengan alias register
	}
	...
	public function verify()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			$this->index(); //di redirect ke function index
		} else {
			...
			$password = password_hash($password, PASSWORD_BCRYPT); //password akan dihash dari string

			$user_data = array(
				'email' => $email,
				'username' => $username,
				'password' => $password,
				'role_id' => 2, //default role adalah customer
			...
			$this->session->set_flashdata('store_flash', 'Successfully register, please login!');
